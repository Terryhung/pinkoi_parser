...
def get_photo_url(url):
    time.sleep(0.3)
    page = DiveToSoup(url)
    photo_url = page.select('div.thumbs > img')
    photo_urls = map(lambda x: x.get("src")[:-9]+"500x0.jpg", photo_url)
    urls = map(lambda x: x.get("src")[:-9], photo_url)
    return zip(urls, photo_urls)
...
def write_into_file(file_name, _list):
    ...
    for urls in _list:
        photo_name = urls[1].split("/")[4] + "_" + urls[1].split("/")[5]
...
def get_photo(url):
    time.sleep(0.1)
    photo_name = url.split('/')[4] + "_" + url.split("/")[5] + ".jpg"
    print photo_name
...
if __name__ == "__main__":
    ...
    for url in url_list:
        ...
        _lists = [val for sublist in lists for val in sublist]
        write_into_file(filename, _lists)
